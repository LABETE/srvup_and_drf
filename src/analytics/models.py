...
class PageView(models.Model):
    ...
    primary_object = GenericForeignKey(
        "primary_content_type", "primary_object_id")
    ...
    secondary_object = GenericForeignKey(
        "secondary_content_type", "secondary_object_id")
...
def page_view_received(sender, *args, **kwargs):
    ...
    if not user.is_authenticated():
        new_page_view = PageView.objects.create(
            path=page_path, timestamp=timezone.now())
    else:
        new_page_view = PageView.objects.create(
            path=page_path, user=user, timestamp=timezone.now())
    if primary_obj:
